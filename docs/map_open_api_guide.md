...
이 앱의 지도 검색은 카카오 지도만 사용한다. 네이버 지도와 서버 프록시 기반 지역 검색은 범위 밖이다.

카카오 검색은 국내만 다루므로 해외 장소는 직접 입력(`provider: manual`)으로 담는다. 직접 입력에는 좌표가 없어 앱 안 지도에 핀이 찍히지 않고, 대신 모든 장소에 구글 지도 열기 action을 둔다. 구글 Maps Platform API는 결제 계정이 필요해 도입하지 않았다.
