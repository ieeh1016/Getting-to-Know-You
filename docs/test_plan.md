...
- `[TRIP-002]` 여행, 항목, 사진은 둘 중 누구든 지울 수 있다. 상대가 담은 것도 지우기 button이 보인다.
- `[TRIP-049]` 타임라인에 끌기 손잡이가 없고, 순서는 시각이 정한다.
- `[TRIP-049]` 사진 날짜 묶기는 둘 다 할 수 있고, 설명은 올린 사람만 고친다.
- `[TRIP-050]` 여행 상세의 상태 카드와 tab 줄 사이에 메모 카드가 늘 있다. 메모가 없어도 숨지 않는다.
- `[TRIP-050]` 메모 sheet는 내용 칸과 링크 칸이 따로다. 링크 없이 내용만으로도 담긴다.
- `[TRIP-050]` 링크를 적은 메모에만 열기가 붙고, 누르면 정규화된 주소로 밖을 연다. scheme 없이 적어도 열린다.
- `[TRIP-050]` 열 수 없는 주소는 담기 전에 sheet에서 알려주고 저장하지 않는다.
- `[TRIP-050]` 메모는 일정·숙소·준비물 목록과 타임라인, 종류 고르기 sheet에 나타나지 않는다.
- `[TRIP-050]` 여행 폼에는 메모 칸이 없고, 예전에 적어둔 여행 메모는 메모 카드 위에 그대로 남는다.
...
- `[TRIP-020]` 여행 사진은 session 로딩에서 읽지 않고 여행 화면에 들어갈 때 한 번만 읽는다.
- `[TRIP-020]` 여행을 지우면 그 여행의 항목과 사진 문서도 함께 지운다.
- `[TRIP-023]` 계획도 `했다`를 표시할 수 있고 숙소는 표시 대상이 아니다.
- `[TRIP-021]` 여행 메모를 저장하고 500자를 넘기지 않는다.
- `[TRIP-046]` 여행 어디에도 금액·낸 사람·경비 단위 입력이 없다. 이미 저장된 문서에 남은 값 때문에 항목 수정이 막히지 않는다.
- `[TRIP-046]` `계획` 폼에 `언젠가, 같이에서 가져오기` 줄이 없다.
- `[TRIP-047]` 계획과 이동도 고치기 sheet 안에서 지울 수 있고, 확인 sheet를 거친다. 담은 사람만 보인다.
- `[TRIP-048]` 실패한 write가 남아 있으면 그 뒤 저장이 성공해도 재시도 안내가 사라지지 않는다.
- `[TRIP-020]` 여행 사진은 여행 하나를 열 때 `tripId`로 걸러 그 여행 것만 읽는다.
- `[TRIP-024]` 새 여행을 저장하면 목록이 아니라 그 여행 상세가 열린다.
- `[TRIP-024]` Home의 다가오는 여행 카드는 목록을 거치지 않고 그 여행 상세로 들어간다.
- `[TRIP-024]` 지목해서 연 여행 표시는 한 번만 쓰이고 그 뒤에는 비워진다.
- `[TRIP-025]` 여행 상세 tab 줄은 스크롤해도 위에 붙어 있다.
- `[TRIP-025]` 3일 이상 여행의 `일정` tab에는 날짜 이동 줄이 보이고, 2일 여행에는 보이지 않는다.
- `[TRIP-026]` `담고 계속 적기`는 sheet를 닫지 않고 저장하며, 무엇을 담았는지 한 줄로 알려준다.
- `[TRIP-026]` 항목 폼 안에서 종류를 바꿀 수 있고, 앞 종류에서만 쓰던 값은 비워진다.
- `[TRIP-026]` 이미 고른 시각을 그 자리에서 지울 수 있다.
- `[TRIP-026]` 숙소도 날짜를 미정으로 둘 수 있다.
- `[TRIP-027]` 지난 여행의 준비물을 가져올 수 있고, 이름이 겹치면 늘어나지 않으며 챙긴 표시는 옮기지 않는다.
- `[TRIP-030]` `계획 중`인 여행의 날짜는 만남 달력에 표시되고, `다녀옴`으로 옮기면 표시가 사라진다.
- `[TRIP-031]` 서버 사진을 받아도 방금 저장한 사진이 사라지지 않는다.
- `[TRIP-031]` 사진은 한 번에 여러 장 고를 수 있고 `maxTripPhotoPickCount`까지만 담는다.
- `[TRIP-031]` 사진 뷰어는 방금 보던 목록의 범위와 순서를 그대로 따른다.
- `[TRIP-032]` 날짜 sheet는 고른 날이 없으면 오늘이 있는 달에서 열린다.
- `[TRIP-032]` 숙소/준비물 tab에서 담기를 누르면 종류 sheet 없이 바로 폼이 열리고, 일정 tab에서는 여전히 종류를 먼저 고른다.
- `[TRIP-032]` 폼 안에서 종류를 바꿔 저장하면 그 종류의 tab으로 옮겨간다.
- `[TRIP-032]` 항목 폼은 제목 다음에 날짜·시각이 오고, 담기 결과는 button과 같은 고정 자리에 보인다.
- `[TRIP-032]` 이어 적을 때 시각 sheet가 방금 고른 시각 근처에서 열리고, 칸 자체는 비어 있다.
- `[TRIP-032]` 이동의 출발지·도착지를 화살표로 뒤집을 수 있고, 이어 담으면 돌아오는 편으로 뒤집혀 채워진다.
- `[TRIP-033]` 폼 sheet는 당겨서 닫히지 않고, 닫아도 적던 내용이 memory draft로 남아 `이어서 쓰기`로 되살아난다. 저장하면 지워지고 Firestore write는 없다.
- `[TRIP-034]` 타임라인의 각 날 끝과 빈 날 줄을 누르면 그 날짜가 골라진 담기 sheet가 열린다. 기존 항목 편집은 날짜가 바뀌지 않는다.
- `[TRIP-035]` 항목의 링크는 `예약 열기`로 열리고, `http`/`https`가 아니면 button을 만들지 않는다. scheme 없는 값도 밖으로 연다.
- `[TRIP-035]` 장소를 붙인 숙소는 주소와 `지도` 열기를 카드와 일정 tab 띠 양쪽에 보여준다.
- `[TRIP-036]` 준비물 8개 이상이면 좁히기 줄이 보이고, 담당으로 좁혀 빠진 미배정 개수를 알려준다. 다른 여행을 열면 필터가 풀리고 write는 없다.
- `[TRIP-036]` 사진 필터에 `날짜 없음` 칸이 미분류가 있을 때만 나타난다.
- `[TRIP-037]` 사진 삭제도 확인 sheet를 거치고, `그대로 두기`를 고르면 남는다.
- `[TRIP-037]` 여행 밖의 삭제(음악, 댓글, 장소, 위시, 종목, 보유, 건의, 프로필 카드)도 확인 sheet를 거친다.
- `[TRIP-038]` 여행을 지우면 목록에서 즉시 사라지고, 진행 중인 사진 읽기를 기다렸다가 문서를 지운다.
- `[TRIP-038]` 항목·사진 삭제가 실패하면 화면에서 되살아나고 재시도 큐에 남는다.
- `[TRIP-039]` 체크한 항목의 안내 문구는 `남겼어요`가 아니라 `챙겼다고/했다고 표시했어요`다.
- `[TRIP-040]` 여행 화면에서 아래로 당기면 세션과 열어둔 여행의 사진을 다시 읽는다.
- `[TRIP-041]` 사진은 사진 tab을 고르거나 일정 tab의 불러오기 줄을 누를 때만 읽는다. 여행을 여는 것만으로는 읽지 않는다.
- `[TRIP-041]` 아직 사진을 안 읽은 여행 카드는 `사진 0`이라고 말하지 않는다.
- `[TRIP-042]` 홈 카드는 지금 시각 다음에 오는 일정을 짚고, 전부 지났으면 마지막 것을 짚는다.
- `[TRIP-042]` 여행의 `오늘`은 기기 로컬 날짜를 따르고, 오늘의 질문과 만남 달력은 KST를 그대로 쓴다.
- `[TRIP-043]` 다녀온 여행에서만 계획에 `했다` 체크가 보인다.
- `[TRIP-044]` `어디에서` picker에서 직접 담기로 만든 장소가 그 자리에서 골라진다.
- `[PLACE-MAPS]` 직접 담기 sheet에서 이름·주소를 적으면 `구글 지도에서 찾아보기`가 눌리고, 저장 전에 그 검색 주소로 밖을 연다. 비어 있으면 눌리지 않는다.
- `[PLACE-RULES]` `firestore.rules`의 장소 생성 검사가 좌표와 provider id를 카카오 장소에만 요구한다. 직접 담은 장소가 거부되지 않는다.
- `[PLACE-RULES]` 소유자 수정이 `mapLink`를 바꿀 수 있다.
- `[PLACE-RULES]` 장소 보드보다 나중에 생긴 필드는 없는 문서에서도 규칙 평가가 오류나지 않는다. 옛 장소를 계획 날짜에 붙일 수 있다.
- `[RULES-EMULATOR]` `tools/firestore_rules_test`가 emulator에 규칙을 올려 실제 쓰기를 검사한다. 계획 탭 붙이기, 관심 표시, 장소 이름 고치기, 직접 담기 생성이 통과하고 space 밖 사용자는 거부된다.
- `[RULES-EMULATOR]` 규칙이 표현식 1000개 한도를 넘지 않는다. 큰 검사를 `||`로 이어 붙이면 데이터와 상관없이 거부된다.
- `[TRIP-045]` 만남 달력의 여행 날은 어느 여행인지 보여주고 그 여행으로 들어간다. 상태 문구도 조율이 아니라 `여행 중인 날`로 읽힌다.
- `[HOME-GUIDE]` 처음 안내서는 실제로 있는 진입점만 소개한다. `우리 선택` 행은 없고 데이트/계획/장소/여행/기억 행이 있다.
- `[MEETING-PLAN]` `이 날 장소 후보` 빈 상태는 보드에 담아둔 곳이 있으면 아래 목록을 가리키고, 보드까지 비었을 때만 장소 탭을 안내한다.
- `[MEETING-AUTOSAVE]` 계획 항목을 담거나 지우거나 순서를 바꾸면 그때마다 `meetingPlans/{dateKey}` 한 문서를 저장한다. `계획 저장` button은 없다.
- `[MEETING-AUTOSAVE]` 계획 입력칸 타이핑은 repository write를 만들지 않는다.
- `[MEETING-AUTOSAVE]` 자동 저장이 돌아도 고치던 줄의 편집 상태와 입력값이 유지된다.
...
- session 로드가 실패하면 setup required가 아니라 연결 실패 화면과 `다시 시도`를 보여준다. 연결 문제에 개발자용 Firebase 설정 안내를 띄우지 않는다.
