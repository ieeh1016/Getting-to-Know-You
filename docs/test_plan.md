...
- `[TRIP-021]` 여행 메모와 경비 단위를 저장하고, 메모는 500자를 넘기지 않는다.
- `[TRIP-022]` 항목의 든 돈이 합계와 사람별 금액으로 모이고, 음수 금액과 우리 둘이 아닌 결제자는 거부한다.
...
- `[TRIP-028]` 새 `계획`을 담을 때 `언젠가, 같이`에 적어둔 것에서 제목을 가져올 수 있다.
- `[TRIP-029]` 든 돈 카드는 낸 사람이 없는 금액을 따로 밝히고, 둘이 낸 금액이 다르면 누가 얼마 더 냈는지 한 줄로 보여준다.
...
- `[TRIP-035]` 항목에 적어둔 금액이 목록과 타임라인에서 바로 읽힌다.
...
- `[TRIP-039]` 순서만 바꾸면 `updatedAt`이 갱신되지 않아 상대 홈에 새 소식이 쌓이지 않는다.
...
- `[TRIP-043]` 위시에서 가져온 계획을 `했다`로 표시하면 상세에서 그 위시를 함께 닫을 수 있고, 닫을 권한이 없는 위시는 목록에 들지 않는다.
