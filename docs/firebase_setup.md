...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    function validAnswerCommentShape(spaceId, commentId) {
      return request.resource.data.keys().hasOnly([
          'questionId',
        ...
        && request.resource.data.commenterProfileId is string
        && request.resource.data.commenterProfileId in get(/databases/$(database)/documents/spaces/$(spaceId)).data.memberIds
        && request.resource.data.answerOwnerProfileId != request.resource.data.commenterProfileId
        ...
        && (
          !request.resource.data.keys().hasAny(['replyBody'])
          || (
            request.resource.data.replyBody is string
            && request.resource.data.replyBody.size() <= 120
          )
        )
        && (
          !request.resource.data.keys().hasAny(['repliedByProfileId'])
          || (
            request.resource.data.repliedByProfileId is string
            && request.resource.data.repliedByProfileId.size() <= 128
            && (
              request.resource.data.repliedByProfileId == ''
              || request.resource.data.repliedByProfileId == request.resource.data.answerOwnerProfileId
            )
          )
        )
        && (
          !request.resource.data.keys().hasAny(['replyCreatedLabel'])
          || (
            request.resource.data.replyCreatedLabel is string
            && request.resource.data.replyCreatedLabel.size() <= 16
          )
        )
        && (
          !request.resource.data.keys().hasAny(['replyEdited'])
          || request.resource.data.replyEdited is bool
        )
    ...
    function blankAnswerCommentReply() {
      return (
          !request.resource.data.keys().hasAny(['replyBody'])
          || request.resource.data.replyBody == ''
        )
        && (
          !request.resource.data.keys().hasAny(['repliedByProfileId'])
          || request.resource.data.repliedByProfileId == ''
        ...
        && (
          !request.resource.data.keys().hasAny(['replyCreatedLabel'])
          || request.resource.data.replyCreatedLabel == ''
        )
        && (
          !request.resource.data.keys().hasAny(['replyEdited'])
          || request.resource.data.replyEdited == false
        );
    }

    function preservesExistingAnswerCommentReply() {
      return (
          !resource.data.keys().hasAny(['replyBody'])
          && blankAnswerCommentReply()
        )
        || (
          resource.data.keys().hasAny(['replyBody'])
          && request.resource.data.replyBody == resource.data.replyBody
          && request.resource.data.repliedByProfileId == resource.data.repliedByProfileId
          && request.resource.data.replyCreatedLabel == resource.data.replyCreatedLabel
          && request.resource.data.replyEdited == resource.data.replyEdited
        );
    }

    function validAnswerCommentOwnerWrite(spaceId, commentId) {
      return validAnswerCommentShape(spaceId, commentId)
        && request.resource.data.commenterProfileId == request.auth.uid
        && request.resource.data.answerOwnerProfileId != request.auth.uid
        && (
          (
            !exists(/databases/$(database)/documents/spaces/$(spaceId)/answerComments/$(commentId))
            && blankAnswerCommentReply()
          )
          || (
            exists(/databases/$(database)/documents/spaces/$(spaceId)/answerComments/$(commentId))
            && preservesExistingAnswerCommentReply()
          )
        );
    }

    function validAnswerCommentReplyWrite(spaceId, commentId) {
      return validAnswerCommentShape(spaceId, commentId)
        && exists(/databases/$(database)/documents/spaces/$(spaceId)/answerComments/$(commentId))
        && resource.data.answerOwnerProfileId == request.auth.uid
        && request.resource.data.questionId == resource.data.questionId
        && request.resource.data.answerOwnerProfileId == resource.data.answerOwnerProfileId
        && request.resource.data.commenterProfileId == resource.data.commenterProfileId
        && request.resource.data.body == resource.data.body
        && request.resource.data.createdLabel == resource.data.createdLabel
        && request.resource.data.edited == resource.data.edited
        && request.resource.data.replyBody.size() > 0
    ...
    function validTripShape(spaceId, tripId) {
      return request.resource.data.keys().hasOnly([
        ...
        // 여행에서 든 돈은 걷어냈다. 키는 이미 저장된 문서에 남아 있어
        // 허용 목록에서 빼면 그 문서들이 더 이상 수정되지 않는다.
        // 새로 쓸 때는 아예 보내지 않으므로 있을 때만 검사한다.
        && (
          !('currencyLabel' in request.resource.data)
          || (
    ...
    function validTripPhotoShape(spaceId, photoId) {
      return request.resource.data.keys().hasOnly([
          'id',
          'tripId',
          'imageDataUrl',
    ...
    function validTripItemShape(spaceId, itemId) {
      return request.resource.data.keys().hasOnly([
        ...
        && request.resource.data.timeLabel.size() <= 5
        && request.resource.data.endDateKey is string
        && request.resource.data.endDateKey.size() <= 10
        && request.resource.data.endTimeLabel is string
        && request.resource.data.endTimeLabel.size() <= 5
        && request.resource.data.transportMode in ['', 'flight', 'train', 'bus', 'car', 'ship', 'walk']
        && request.resource.data.fromLabel is string
        ...
        && request.resource.data.assigneeProfileId is string
        && request.resource.data.assigneeProfileId.size() <= 120
        && request.resource.data.sortOrder is int
        && request.resource.data.sortOrder >= 0
        && request.resource.data.sortOrder <= 999
        // 여행에서 든 돈은 걷어냈다. 이미 저장된 문서에 남은 값 때문에
        // 키는 허용 목록에 남기고, 있을 때만 검사한다.
        && (
        ...
        && (
          !('paidByProfileId' in request.resource.data)
          || (
            request.resource.data.paidByProfileId is string
            && request.resource.data.paidByProfileId.size() <= 120
          )
        )
        // 계획에 위시를 걸어두는 기능은 걷어냈다. 이미 저장된 문서에 남은
        // 값 때문에 키는 허용 목록에 남기고, 있을 때만 검사한다.
        && (
          !('wishId' in request.resource.data)
          || (
    ...
    function validMemoryCardResponseShape(spaceId, responseId) {
      return request.resource.data.keys().hasOnly([
        ...
        && request.resource.data.correctionText.size() <= 2000
        && (
          (
    ...
    function musicNoteExists(spaceId, noteId) {
      return exists(/databases/$(database)/documents/spaces/$(spaceId)/musicNotes/$(noteId));
    }

    function validMusicNoteCommentShape(spaceId, commentId) {
      return request.resource.data.keys().hasOnly([
          'id',
          'musicNoteId',
          'body',
        ...
        && request.resource.data.musicNoteId is string
        && musicNoteExists(spaceId, request.resource.data.musicNoteId)
        && request.resource.data.body is string
        && request.resource.data.body.size() > 0
        && request.resource.data.body.size() <= 180
        && request.resource.data.createdByProfileId is string
        && request.resource.data.createdByProfileId in get(/databases/$(database)/documents/spaces/$(spaceId)).data.memberIds
        && request.resource.data.createdLabel is string
        && request.resource.data.createdLabel.size() <= 16
    ...
    function validNewMusicNoteComment(spaceId, commentId) {
      return validMusicNoteCommentShape(spaceId, commentId)
        && request.resource.data.createdByProfileId == request.auth.uid
        && request.resource.data.edited == false;
    }

    function validMusicNoteCommentOwnerEdit(spaceId, commentId) {
      return validMusicNoteCommentShape(spaceId, commentId)
        && exists(/databases/$(database)/documents/spaces/$(spaceId)/musicNoteComments/$(commentId))
        && resource.data.createdByProfileId == request.auth.uid
        && request.resource.data.musicNoteId == resource.data.musicNoteId
        && request.resource.data.createdByProfileId == resource.data.createdByProfileId
        && request.resource.data.createdLabel == resource.data.createdLabel
    ...
    function validSharedPlace(spaceId, placeId) {
      return request.resource.data.keys().hasOnly([
        ...
        // `mapLink`는 장소 보드가 생긴 뒤에 추가된 필드다. 그 전에 담은
        // 문서에는 아예 없어서, 무조건 읽으면 규칙 평가 자체가 오류가 된다.
        // Firestore는 오류를 거부로 처리하므로 뒤에 오는 `||` 갈래도
        // 함께 죽는다. 그래서 옛 장소는 계획에 붙이는 것조차 막혔다.
        && (
          !request.resource.data.keys().hasAny(['mapLink'])
          || (
    ...
    function validNewSharedPlace(spaceId, placeId) {
      return validSharedPlace(spaceId, placeId)
        && request.resource.data.createdByProfileId == request.auth.uid
        // 카카오 검색으로 담은 곳은 늘 id와 좌표가 따라온다. 직접 담은
        // 곳에는 둘 다 없다. 해외 여행은 거의 전부 이 경로라, 좌표를
        // 무조건 요구하면 직접 담기가 통째로 막힌다.
        && (
    ...
    function sharedPlaceOwnerEditShape() {
      return resource.data.createdByProfileId == request.auth.uid
    ...
    function sharedPlaceInterestShape() {
      return requestSharedPlaceMeetingPlanLinks() == existingSharedPlaceMeetingPlanLinks()
    ...
    function sharedPlaceMeetingLinkShape() {
    ...
    // 규칙은 요청 하나에 표현식 1000개까지만 평가한다. 아래 세 모양은
    // 저마다 `validSharedPlace`를 다시 부르곤 했는데, 그러면 셋을 이어 붙인
    // 것만으로 한도를 넘어 데이터와 상관없이 거부됐다. 큰 검사는 한 번만
    // 부르고, 갈래별로 다른 것은 값싼 모양 검사로만 가른다.
    function validSharedPlaceFullUpdate(spaceId, placeId) {
      return validSharedPlace(spaceId, placeId)
        && request.resource.data.id == resource.data.id
        && request.resource.data.createdByProfileId == resource.data.createdByProfileId
        && (
          sharedPlaceOwnerEditShape()
          || sharedPlaceInterestShape()
          || sharedPlaceMeetingLinkShape()
        );
    }
    ...
    match /spaces/{spaceId} {
      ...
      match /answerComments/{commentId} {
        ...
        allow create, update: if isSpaceMember(spaceId)
          && (
            validAnswerCommentOwnerWrite(spaceId, commentId)
            || validAnswerCommentReplyWrite(spaceId, commentId)
          );
        allow delete: if false;
      }
      ...
      match /musicNoteComments/{commentId} {
        allow read: if isSpaceMember(spaceId);
        allow create: if isSpaceMember(spaceId)
          && validNewMusicNoteComment(spaceId, commentId);
        allow update: if isSpaceMember(spaceId)
          && validMusicNoteCommentOwnerEdit(spaceId, commentId);
        allow delete: if isSpaceMember(spaceId)
          && resource.data.createdByProfileId == request.auth.uid;
      }
      ...
      match /sharedPlaces/{placeId} {
        ...
        // 규칙은 요청 하나에 표현식 1000개까지만 평가한다. 아래 검사들은
        // 대부분 `validSharedPlace`를 통째로 다시 부르는 큰 식이라, 여러 개를
        // `||`로 이어 붙이면 데이터와 상관없이 한도를 넘어 거부된다.
        // `meetingPlanLinks`처럼 몇 필드만 바꾸는 patch가 가장 잦으므로
        // 그 갈래를 맨 앞에 두어 무거운 검사에 닿기 전에 끝나게 한다.
        allow update: if isSpaceMember(spaceId)
          && (
            validSharedPlaceMeetingPatchUpdate(spaceId, placeId)
            || validSharedPlaceFullUpdate(spaceId, placeId)
          );
        allow delete: if isSpaceMember(spaceId)
          && resource.data.createdByProfileId == request.auth.uid;
      }
...
GitHub Actions workflow는 `google-github-actions/auth@v2`에
`FIREBASE_SERVICE_ACCOUNT` JSON을 넘겨 Application Default Credentials 파일을
만들고, `GOOGLE_APPLICATION_CREDENTIALS`를 포함한 인증 환경변수를 export한다.
따라서 CI runner에서 `firebase login`을 실행하거나 legacy `FIREBASE_TOKEN`을
추가할 필요가 없다.
