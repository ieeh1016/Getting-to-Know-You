...
| Trips | `Trip`, `TripItem`, `TripDay`, `TripPhoto` | `trips/{tripId}`, `tripItems/{itemId}`, `tripPhotos/{photoId}` | 두 member 모두 여행과 항목을 create/edit할 수 있고 삭제는 creator만 한다. 항목은 `tripId`로 여행을 가리키며, 숙소만 날짜 범위를, 이동만 수단/경로를, 준비물만 체크 상태를 쓴다. 사진은 Storage 없이 data URI로 담고 올린 사람만 지운다. |
