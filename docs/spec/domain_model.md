...
| Push notifications | `PushNotificationSetupState`, `PushNotificationIntent` | `users/{uid}/notificationSettings/push`, `users/{uid}/notificationTokens/{tokenId}`, `notificationEvents` | member는 자신의 push setting/token만 쓴다. Cloud Functions는 shared memory event만 발송하고 private content를 payload에 넣지 않는다. |
...
- Push notification은 Home unread activity를 대체하지 않는다. FCM payload는 route/target id만 담고, 실제 detail은 앱이 Firestore read model에서 다시 읽는다.
