...
| Push notifications | `PushNotificationSetupState`, `PushNotificationIntent`, `activityEvents` | `users/{uid}/notificationSettings/push`, `users/{uid}/notificationTokens/{tokenId}`, `activityEvents`, `notificationEvents` | member는 자신의 push setting/token만 쓴다. client는 명시적 저장 동작 성공 시 safe metadata만 담은 activity event를 만들고, Cloud Functions는 상대 member에게 1회 발송한다. private content와 긴 본문은 payload에 넣지 않는다. |
