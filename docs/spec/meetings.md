...
- 계획 항목은 **담거나, 고치거나, 지우거나, 순서를 바꾸면 그 자리에서 저장한다.** 따로 `계획 저장`을 누르지 않는다. 한 줄 적을 때마다 저장을 다시 누르게 하면 손이 너무 많이 간다.
- 타이핑은 저장하지 않는다. `추가`/`완료`를 누르는 것, 지우는 것, 순서를 바꾸는 것만 명시적인 동작이고 그때마다 `meetingPlans/{dateKey}` 문서 하나를 쓴다.
- 자동 저장은 고치던 줄을 건드리지 않는다. 저장했다고 편집을 취소해 버리면 순서를 바꾸는 사이에 적던 것이 사라진다.
- 아직 만나는 날로 정하지 않은 날짜에서는 저장하지 않고 `먼저 약속에서 만나는 날을 정해주세요`를 그대로 보여준다. 조용히 넘기면 적은 것이 저장된 줄 알고 화면을 떠난다.
- 저장 결과는 기존 저장 상태 줄에 그대로 나온다. 실패하면 `다시 시도`가 붙는다.
