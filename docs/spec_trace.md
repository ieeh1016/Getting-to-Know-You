...
| `PUSH-001` | Push notifications are opt-in, token owner-scoped, and sent only for shared memory events without private payload text. | `docs/spec/notifications.md`, `docs/spec/firestore.md`, `docs/firebase_setup.md` | Unit Tests, Widget Tests, Manual Checks | `test/src/ui/alagagi_auth_gate_test.dart`: `push notifications can be enabled from my screen`; `test/harness_contract_test.dart`: `push notification rules and functions keep tokens owner scoped` |
