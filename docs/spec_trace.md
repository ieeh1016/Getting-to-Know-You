...
| `Q-CATALOG-001` | Active daily question catalog stays sequential from day one, keeps ids unique across versions, and avoids long-term pressure language. | `docs/spec/questions.md` | Unit Tests | `test/src/domain/alagagi_controller_test.dart`: `active catalog days stay sequential from day one`; `question ids never repeat across catalog versions`; `daily question catalog avoids long-term pressure language` |
| `Q-CATALOG-002` | Every day up to today keeps its original question, the next set starts tomorrow, and the cutover is derived from the real start date. | `docs/spec/questions.md` | Unit Tests | `test/src/domain/alagagi_controller_test.dart`: `every day up to today keeps its original question`; `new questions start the day after today`; `cutover follows the real start date instead of a fixed day` |
| `Q-CATALOG-003` | Answers to questions that left the active order stay visible in the archive with their original text. | `docs/spec/questions.md` | Unit Tests | `test/src/domain/alagagi_controller_test.dart`: `answers to retired questions stay visible in the archive` |
...
| `MEETING-003` | Long schedule notes stay readable: the person row clamps to two lines and offers a 전체 보기 sheet. | `docs/spec/meetings.md` | Widget Tests | `test/src/ui/alagagi_app_test.dart`: `long meeting note stays readable behind a 전체 보기 cue` |
...
| `PROFILE-002` | Profile card slot catalog v3 provides 20 prompts across 취향/하루/대화/함께 and drops retired slot ids from the UI. | `docs/spec/profile_cards.md` | Unit Tests | `test/src/domain/alagagi_controller_test.dart`: `profile card catalog provides expanded category packs` |
...
| `TRIP-001` | Trip duration is derived from the two dates and an end date before the start date is rejected. | `docs/spec/trips.md` | Unit Tests, Widget Tests | `test/src/domain/trip_planning_test.dart`: `two night trip reads as 2박 3일 and lists every day`; `end date before start date is rejected without storing a trip`; `test/src/ui/alagagi_app_test.dart`: `trip screen plans a trip and tracks packing checks` |
| `TRIP-002` | Trip items stay inside the trip range, only packing items carry check state, and deletion is creator-only. | `docs/spec/trips.md`, `docs/spec/domain_model.md` | Unit Tests | `test/src/domain/trip_planning_test.dart`: `trip items only accept dates inside the trip`; `shortening a trip moves out-of-range item dates back to undecided`; `check state only toggles for packing items`; `only the creator can delete a trip or an item` |
| `TRIP-003` | Trip and trip item saves each write a single document and browsing creates no write. | `docs/spec/trips.md`, `docs/spec/firestore.md` | Widget Tests, Firestore Free Plan Budget Checks | `test/src/ui/alagagi_app_test.dart`: `trip screen plans a trip and tracks packing checks` |
| `TRIP-004` | Trip detail opens on a day-by-day timeline ordered by time, and editing from the timeline updates the original item. | `docs/spec/trips.md` | Unit Tests, Widget Tests | `test/src/domain/trip_planning_test.dart`: `timeline groups items by day and orders them by time`; `day header labels read as 1일차 and a dated weekday`; `time label is rejected unless it reads as HH:mm`; `test/src/ui/alagagi_app_test.dart`: `trip timeline orders a day by time and opens an item to edit` |
| `UI-CARD-001` | Paper cards use exactly two shapes: standard and compact. | `docs/code_structure.md`, `docs/design/README.md` | Widget Tests | `test/src/shared/paper_card_geometry_test.dart`: `default paper card uses the standard card shape`; `compact paper card uses the dense list card shape` |
...
| `MEMORY-003` | Memory card title/body limits are 80/2000 and long bodies clamp to a readable-detail cue in the list. | `docs/spec/memory_cards.md`, `docs/spec/firestore.md` | Widget Tests | `test/src/domain/memory_card_limits_test.dart`: `a body at the documented limit is accepted`; `a body one character over the limit is rejected`; `test/src/ui/alagagi_app_test.dart`: `memory card long body clamps and opens the readable detail sheet` |
| `PUSH-001` | Push notifications remain token owner-scoped and private-payload safe, but the server path is dormant by default for Spark plan. | `docs/spec/notifications.md`, `docs/spec/firestore.md`, `docs/firebase_setup.md` | Unit Tests, Widget Tests, Manual Checks | `test/src/ui/alagagi_auth_gate_test.dart`: `push notifications stay hidden in Spark-compatible mode`; `test/harness_contract_test.dart`: `push notification pipeline stays dormant for Spark plan` |
